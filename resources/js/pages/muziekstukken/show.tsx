...
function useGroupedTypes(instrumentTypes: InstrumentType[], parts: Part[]) {
    return useMemo(() => {
        ...
        const familyMap = new Map<string, { type: InstrumentType; parts: Part[] }[]>();
...
function PartsOverview({ parts, instrumentTypes, t }: { parts: Part[]; instrumentTypes: InstrumentType[]; t: (key: string) => string }) {
    const { conductorParts, families } = useGroupedTypes(instrumentTypes, parts);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {families.filter(({ types }) => types.some(({ parts: tp }) => tp.length > 0)).map(({ familyName, types }) => (
                    <div key={familyName} className="space-y-2">
                        <h3 className="text-sm font-semibold">{familyName}</h3>
                        <ul className="space-y-1">
                            {types.filter(({ parts: tp }) => tp.length > 0).map(({ type, parts: typeParts }) => {
                                const totalBought = typeParts.reduce((sum, p) => sum + (p.amount_bought ?? 1), 0);
                                return (
                                    <li
                                        key={type.id}
                                    ...
                                    >
                                        ...
                                        {typeParts.length === 1 && typeParts[0].download_url && (
                                            <a href={typeParts[0].download_url} download>
                                                <Download className="size-4 text-muted-foreground hover:text-foreground" />
                                                <span className="sr-only">{t('Download')}</span>
                                            </a>
                                        )}
...
export default function Show({ piece, parts, instrumentTypes, canEdit }: Props) {
    ...
    const visibleUsages = showPreviousUsages ? [...currentUsages, ...previousUsages] : currentUsages;
            ...
            <div className="space-y-10 p-6">
                ...
                <section className="space-y-6">
                    ...
                    <dl className="grid gap-4 sm:grid-cols-2">
                        ...
                        <Field label={t('Difficulty')} value={piece.difficulty} />
                        <Field label={t('Bought for')} value={piece.bought_for} />
                        <Field label={t('Buy date')} value={piece.buy_date} />
                        <Field label={t('Archive number')} value={piece.archive_number} />
                        <Field label={t('Music type')} value={piece.music_type} />
                    </dl>

                    {piece.genre && piece.genre.length > 0 && (
                        <div className="space-y-1">
                            <dt className="text-sm text-muted-foreground">{t('Genre')}</dt>
                    ...
                    {piece.notes && (
                        <div className="space-y-1">
                            ...
                            <dd className="text-sm whitespace-pre-line">{piece.notes}</dd>
                        </div>
                    )}
                    ...
                    {visibleUsages.length > 0 && (
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <dt className="text-sm text-muted-foreground">{t('In use by')}</dt>
                                {previousUsages.length > 0 && (
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => setShowPreviousUsages((v) => !v)}
                                    >
                                        <History />
                                        {showPreviousUsages ? t('Hide previous usage') : t('Show previous usage')}
                                    </Button>
                                )}
                            </div>
                            <dd className="flex flex-wrap gap-2">
                                {visibleUsages.map((u) => (
                                    <div key={u.id} className="flex items-center gap-1.5">
                                        <Badge variant={u.tot ? 'outline' : 'secondary'}>
                                            {u.orchestra.name}
                                        </Badge>
                                        <span className="text-xs text-muted-foreground">
                                            {u.van ?? '?'} &ndash; {u.tot ?? t('present')}
                                        </span>
                                        {u.details && (
                ...
                <section className="space-y-6">
                    ...
                    <Tabs defaultValue="parts">
                        <TabsList>
                            ...
                            <TabsTrigger value="parts-overview">{t('Parts overview')}</TabsTrigger>
                        </TabsList>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                ...
                                                <TableHead>{t('Amount bought')}</TableHead>
                                        ...
                                        <TableBody>
                                            {parts.map((part) => (
                                                <TableRow key={part.id}>
                                                    ...
                                                    <TableCell>
                                                        {part.voice ?? <span className="text-muted-foreground">&mdash;</span>}
                                                    </TableCell>
                                                    <TableCell>
                                                        {part.amount_bought ?? <span className="text-muted-foreground">&mdash;</span>}
                                                    </TableCell>
                                                    <TableCell>
                                                        {part.is_conductor ? (
                                                            <Badge>{t('Yes')}</Badge>
                        ...
                        <TabsContent value="parts-overview">
                            <PartsOverview parts={parts} instrumentTypes={instrumentTypes} t={t} />
                        </TabsContent>
                    </Tabs>
                </section>
            </div>
